...
start_services() {
    ...
    docker compose --env-file .env -f docker/docker-compose-lake.yaml up -d
    sleep 5  # Allow services to initialize
    
    # Step 2: Start Trino query engine (coordinator + workers)
    echo "Starting Trino query engine..."
    docker compose --env-file .env -f docker/docker-compose-trino.yaml up -d
    ...
    # load_dbt_seed_data
    # Run `docker compose --env-file .env.development up -d` to start services with env variables
}
...
stop_services() {
    ...
    docker compose -f docker/docker-compose-trino.yaml down -v
    
    echo "Stopping data lake services..."
    docker compose -f docker/docker-compose-lake.yaml down -v
